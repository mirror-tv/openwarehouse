...
module.exports.GCSAdapter = class {
    ...
    async save({ stream, filename, mimetype, encoding, id }) {
        ...
        //     const duration = this.getDuration(stream)
        ...
        try {
            ...
            console.log("MIMEINFO GOT: ",mimeInfo)
        } catch (err) { console.log(err) }

        mimedata.mimeInfo = mimeInfo
        console.log("MIME OK")
        return { id, filename, mimedata }
    }
    ...
    async uploadFile(stream, file, baseName) {
        return new Promise((resolve, reject) => {
            ...
            //console.log("UPLOAD COMPLETED") //此行出現不代表上行有觸發
        })
