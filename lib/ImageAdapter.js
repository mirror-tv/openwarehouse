...
class ImageAdapter extends GCSAdapter {
    ...
    sync_save() {
        return new Promise(async (resolve, reject) => {
            ...
            try {
				...
				console.log("before compress" + Date.now())
                await this.compressAndRenameImage()
				console.log("after compress" + Date.now())
				console.log("before apiData" + Date.now())
                await generateImageApiData(this.imageNameList, this.apiData)
				console.log("after apiData" + Date.now())
				console.log("before resize" + Date.now())
                await saveVariousSizeImageToLocal(
                    this.newFilename,
                    this.apiData
                )
				console.log("after resize" + Date.now())
    ...
    compressAndRenameImage() {
        return new Promise(async (resolve, reject) => {
            try {
				console.log("before compress - change quality: " + Date.now())
                const image = await Jimp.read(
                    `./public/images/${this.originalFileName}`
                ).then((image) => {
                    return image.quality(70)
                })
				console.log("after compress - change quality: " + Date.now())

				console.log("before compress - write new file: " + Date.now())
                await image.writeAsync(`./public/images/${this.newFilename}`)
				console.log("after compress - write new file: " + Date.now())
